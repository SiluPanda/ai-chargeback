...
function getPrice(model, customPricing) {
    if (customPricing?.[model])
        return customPricing[model];
    if (exports.BUILT_IN_PRICING[model])
        return exports.BUILT_IN_PRICING[model];
    const stripped = model.replace(DATE_SUFFIX, '');
    if (stripped !== model) {
        if (customPricing?.[stripped])
            return customPricing[stripped];
        if (exports.BUILT_IN_PRICING[stripped])
            return exports.BUILT_IN_PRICING[stripped];
    }
    return undefined;
}
